...
import { Callout } from '@/components/ui/shadcn/callout'
import { CodeBlockCommand } from '@/components/ui/shadcn/code-block-command'
import { CodeCollapsibleWrapper } from '@/components/ui/shadcn/code-collapsible-wrapper'
import { CodeTabs } from '@/components/ui/shadcn/code-tabs'
import { CopyButton } from '@/components/ui/shadcn/copy-button'
import { getIconForLanguageExtension } from '@/components/ui/shadcn/icons'
...
export const shadcnMDXComponents = {
    ...
    h2: ({ className, ...props }: React.ComponentProps<'h2'>) => {
        return (
            <h2
                ...
                className={cn(
                    'font-heading mt-12 scroll-m-28 text-2xl font-medium tracking-tight first:mt-0 lg:mt-12 [&+p]:!mt-4 *:[code]:text-2xl',
                    className
                )}
                {...props}
            />
    ...
    table: ({ className, ...props }: React.ComponentProps<'table'>) => (
        <Card className="mt-4 p-0 rounded-lg bg-transparent overflow-hidden border-none">
            <CardContent className="p-0">
                <Table className={cn('bg-code', className)} {...props} />
            </CardContent>
        </Card>
    ),
    thead: ({ className, ...props }: React.ComponentProps<'thead'>) => <TableHeader className={className} {...props} />,
    tbody: ({ className, ...props }: React.ComponentProps<'tbody'>) => <TableBody className={className} {...props} />,
    tr: ({ className, ...props }: React.ComponentProps<'tr'>) => <TableRow className={className} {...props} />,
    th: ({ className, ...props }: React.ComponentProps<'th'>) => <TableHead className={cn('px-3', className)} {...props} />,
    td: ({ className, ...props }: React.ComponentProps<'td'>) => <TableCell className={cn('px-3', className)} {...props} />,
    ...
    code: ({
        ...
        if (typeof props.children === 'string') {
            return (
                <code
                    className={cn('bg-muted relative rounded-md px-[0.3rem] py-[0.2rem] font-mono text-[0.8rem] outline-none', className)}
                    {...props}
                />
            )
        }
